...
> 5. **CloudTrail** - This is configured with `event_selector` to capture _data events_ (file access) on our monitored bucket. By default, CloudTrail only logs management events like creating/deleting buckets. We need data events to see who accessed which files.
...
- Make sure you're clicking the link in the _newest_ email.
